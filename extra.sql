...
select ad,soyad,concat(left(ad,1) ,'.',left(soyad,1),'.') as ad_soyad from customer;


-- soru:  5 ve 5 karakterden büyük olan isimleri MORRIS -> MRS şeklinde yazdırınız.
-- yani 1. , 3. ve 5. karakterleri alınız 

select concat( left(ad,1),right(left(ad,3),1),right(left(ad,5),1)) as harfli from customer where length(ad)>5;
