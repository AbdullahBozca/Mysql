...
------------------------------------------------------------------------------*/ 
 SELECT s.sirket_isim, sp.siparis_id, sp.siparis_tarihi, sp.sirket_id
from sirketler s left join siparisler sp
on s.sirket_id=sp.sirket_id;
	
 SELECT s.sirket_isim, sp.siparis_id, sp.siparis_tarihi, sp.sirket_id
from siparisler sp right join sirketler s
on s.sirket_id=sp.sirket_id;    
...
from sirketler s right join siparisler sp 
on s.sirket_id=sp.sirket_id; 
...
union
select sir.sirket_isim, sip.siparis_id,sip.siparis_tarihi, sip.sirket_id
from sirketler sir right join siparisler sip 
on sir.sirket_id=sip.sirket_id;
...
from bolumler b join personel p on b.bolum_id=p.bolum_id where bolum_isim in('satis','muhasebe') order by b.bolum_isim,p.personel_isim;	
...
------------------------------------------------------------------------------*/  
SELECT 
    p.personel_isim, b.bolum_isim
FROM
    personel p
        RIGHT JOIN
    bolumler b ON b.bolum_id = p.bolum_id
WHERE
    bolum_isim IN ('satis' , 'isletme', 'depo')
  ...
  NOT: calisani olmasa bile bolum ismi gosterilmelidir.
------------------------------------------------------------------------------*/ 
select b.bolum_isim,p.personel_isim,p.maas from bolumler b left join personel p ON b.bolum_id = p.bolum_id order by b.bolum_isim desc, p.maas;
...
WHERE
    p.maas > 2000
        AND b.bolum_isim IN ('satis' , 'mudurluk')
